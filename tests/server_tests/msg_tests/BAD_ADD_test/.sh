...
msg=$(cat tests/server_tests/msg_tests/BAD_ADD_test/.in)

printf "$msg" | ncat localhost 1024
sleep 0.2

echo "au().(!(*!^&@#(*^.com" | ncat localhost 1024
sleep 0.2
...
echo "DIFF:"
diff tests/tmp.actual.out tests/server_tests/msg_tests/BAD_ADD_test/.out
echo